...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* TABEL AHLI WARIS */
    .ahli-waris-table { width: 100%; border-collapse: collapse; margin-bottom: 16px; font-size: 12pt; }
    .ahli-waris-table th,
    .ahli-waris-table td { border: 1px solid #000; padding: 5px 8px; vertical-align: top; }
    .ahli-waris-table th { text-align: center; }
    .ahli-waris-table .center { text-align: center; }
...
<body>
    ...
    <tr>
      ...
      <td class="kop-center">
        ...
        <div class="kop-line-xs">
          ...
          Telp. {{ formatContactAW($config->contact ?? '') }}
  ...
  <div class="surat-title">
    <div class="surat-title-text">SURAT KETERANGAN AHLI WARIS</div>
  ...
  {{-- TABEL AHLI WARIS --}}
  @php
    // ahli_waris disimpan sebagai JSON string di detail, decode di sini
    $ahliWarisList = [];
    if (!empty($letter->ahli_waris)) {
      $decoded = is_array($letter->ahli_waris)
        ? $letter->ahli_waris
        : json_decode($letter->ahli_waris, true);
      $ahliWarisList = $decoded ?? [];
    }
  @endphp

  @if (count($ahliWarisList))
    <table class="ahli-waris-table">
      <thead>
        <tr>
      ...
      <tbody>
        @foreach ($ahliWarisList as $i => $item)
          <tr>
  ...
  <div class="surat-isi">
    <p>
        Adalah benar pasangan dan anak kandung sebagai ahli waris dari almarhum :
    </p>    
  </div>
  {{-- DATA PEMOHON / PEWARIS --}}
  <table class="data-table">
    ...
    <tr>
      <td>Alamat</td><td>:</td><td>{{ $letter->alamat ?? '—' }}</td>
    </tr>
  </table>
  ...
  <div class="surat-isi">
    <p>
      Surat keterangan ini dibuat berdasarkan keterangan dan catatan yang
      ada pada kami.
    </p>
    <p>
      Demikian surat keterangan ini kami buat untuk dapat dipergunakan
      seperlunya dan sebagaimana mestinya.
    </p>
  </div>
